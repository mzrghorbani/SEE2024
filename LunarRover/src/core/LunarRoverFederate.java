...
public class LunarRoverFederate extends SEEAbstractFederate implements Observer {
	...
	protected void doAction() {
		...
		this.lunarRover.info(currentReferenceFrame);

	    Position currentPos = this.lunarRover.getPosition();
	    double x = currentPos.getX() - 2.9;  // y
	    double y = currentPos.getY() - 2.32; // x
	    double z = currentPos.getZ() + 1.9;  // z
   
	    Quaternion currentQuat = this.lunarRover.getQuaternion();
	    double w = currentQuat.getW(); 
	    double qx = currentQuat.getX(); 
	    double qy = currentQuat.getY();
	    double qz = currentQuat.getZ() + 1;
	    
	    this.lunarRover.updateState(x, y, z, w, qx, qy, qz);

		try {

			super.updateElement(this.lunarRover);
			System.out.println(this.lunarRover.getState());
	...
	public void update(Observable arg0, Object arg1) {
		...
		System.out.println("LunarRover has received an update");
