...
public class LunarSpacePortFederate extends SEEAbstractFederate implements Observer {

	private static final int MAX_WAIT_TIME = 10000;
	
	
	private LunarSpacePort lunarSpacePort = null;
	...
	public LunarSpacePortFederate(SEEAbstractFederateAmbassador seefedamb, LunarSpacePort lunarSpacePort) {
		super(seefedamb);
		this.lunarSpacePort  = lunarSpacePort;
	...
	public void configureAndStart(Configuration config) throws ConnectionFailed, InvalidLocalSettingsDesignator, UnsupportedCallbackModel, 
		...
		if(!SynchronizationPoint.INITIALIZATION_STARTED.isAnnounced()){
			 ...
			 */
			super.publishElement(lunarSpacePort, "LunarSpacePort");
	...
	protected void doAction() {
		...
		this.lunarSpacePort.info(currentReferenceFrame);

	    Position currentPos = this.lunarSpacePort.getPosition();
	    double x = currentPos.getX() - .9;  // y
	    double y = currentPos.getY() - .32; // x
	    double z = currentPos.getZ() + .5;  // z
	    
	    Quaternion currentQuat = this.lunarSpacePort.getQuaternion();
	    double w = currentQuat.getW(); 
	    double qx = currentQuat.getX(); 
	    double qy = currentQuat.getY();
	    double qz = currentQuat.getZ() + 1;
	    
	    this.lunarSpacePort.updateState(x, y, z, w, qx, qy, qz);

		try {

			super.updateElement(this.lunarSpacePort);
			System.out.println(this.lunarSpacePort.getState());
	...
	public void update(Observable arg0, Object arg1) {
		...
		System.out.println("LunarSpacePort has received an update");
		if(arg1 instanceof ExecutionConfiguration){
            ...
            currentReferenceFrame = (ReferenceFrame) arg1;
            System.out.println("Frame: " + currentReferenceFrame.getFrame());
		}
	}
